import { serve } from "https://deno.land/std@0.168.0/http/server.ts";
import { createClient, SupabaseClient } from "https://esm.sh/@supabase/supabase-js@2";

const corsHeaders = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type",
};

async function pushLine(to: string, messages: object[], accessToken: string): Promise<boolean> {
  const res = await fetch("https://api.line.me/v2/bot/message/push", {
    method: "POST",
    headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
    body: JSON.stringify({ to, messages }),
  });
  if (!res.ok) {
    const body = await res.text();
    console.error(`LINE push failed ${res.status}: ${body}`);
    return false;
  }
  return true;
}

/** Look up LINE user ID via line_users (registration) then line_employee_mapping (manual) */
async function resolveLineId(sb: SupabaseClient, userId: string): Promise<string | null> {
  const { data: lu } = await sb.from("line_users")
    .select("line_user_id")
    .eq("user_id", userId)
    .eq("is_verified", true)
    .maybeSingle();
  if (lu?.line_user_id) return lu.line_user_id;

  const { data: mapping } = await sb.from("line_employee_mapping")
    .select("line_user_id")
    .eq("user_id", userId)
    .eq("is_verified", true)
    .maybeSingle();
  return mapping?.line_user_id ?? null;
}

/** Today's date string in Asia/Taipei, e.g. "2026-04-02" */
function todayTaipei(): string {
  return new Date().toLocaleDateString("sv-SE", { timeZone: "Asia/Taipei" });
}

function taskShortId(id: string) {
  return id.slice(0, 8);
}

function actionFooter(taskId: string, confirmationRequired: boolean) {
  const sid = taskShortId(taskId);
  const rightButton = confirmationRequired
    ? {
        type: "button", style: "primary", height: "sm", color: "#8b5cf6",
        action: { type: "message", label: "🔐 請求確認", text: `/任務 ${sid} 請求確認` },
      }
    : {
        type: "button", style: "primary", height: "sm", color: "#16a34a",
        action: { type: "message", label: "✅ 任務完成", text: `/任務 ${sid} 完成` },
      };
  return {
    type: "box", layout: "horizontal", spacing: "sm", paddingAll: "14px",
    contents: [
      {
        type: "button", style: "secondary", height: "sm",
        action: { type: "message", label: "📝 更新備註", text: `/任務 ${sid} 更新 ` },
      },
      rightButton,
    ],
  };
}

type TaskWithWorkflow = {
  id: string;
  title: string;
  due_date: string | null;
  confirmation_required: boolean;
  workflow_instance?: { name: string } | null;
};

function workflowName(task: TaskWithWorkflow): string | null {
  return task.workflow_instance?.name ?? null;
}

function buildReminderFlex(task: TaskWithWorkflow) {
  const wfName = workflowName(task);
  const dueLabel = task.due_date
    ? new Date(task.due_date).toLocaleString("zh-TW", { timeZone: "Asia/Taipei", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" })
    : "未設定";
  return {
    type: "flex",
    altText: `⏰ 提醒：任務「${task.title}」即將到期`,
    contents: {
      type: "bubble",
      size: "kilo",
      header: {
        type: "box", layout: "vertical", backgroundColor: "#F59E0B", paddingAll: "14px",
        contents: [
          ...(wfName ? [{ type: "text", text: wfName, size: "xs", color: "#FFF8E1" }] : []),
          { type: "text", text: "⏰ 任務提醒", weight: "bold", color: "#FFFFFF", size: "md" },
        ],
      },
      body: {
        type: "box", layout: "vertical", spacing: "sm", paddingAll: "14px",
        contents: [
          { type: "text", text: task.title, weight: "bold", size: "md", wrap: true },
          { type: "text", text: `到期時間：${dueLabel}`, size: "sm", color: "#666666" },
        ],
      },
      footer: actionFooter(task.id, task.confirmation_required),
    },
  };
}

function buildOverdueFlex(task: TaskWithWorkflow & { due_date: string }, daysOverdue: number) {
  const wfName = workflowName(task);
  const dueLabel = new Date(task.due_date).toLocaleString("zh-TW", { timeZone: "Asia/Taipei", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" });
  return {
    type: "flex",
    altText: `🔴 逾期：任務「${task.title}」已超過到期時間 (${daysOverdue}天)`,
    contents: {
      type: "bubble",
      size: "kilo",
      header: {
        type: "box", layout: "vertical", backgroundColor: "#EF4444", paddingAll: "14px",
        contents: [
          ...(wfName ? [{ type: "text", text: wfName, size: "xs", color: "#FEE2E2" }] : []),
          { type: "text", text: "🔴 任務逾期通知", weight: "bold", color: "#FFFFFF", size: "md" },
        ],
      },
      body: {
        type: "box", layout: "vertical", spacing: "sm", paddingAll: "14px",
        contents: [
          { type: "text", text: task.title, weight: "bold", size: "md", wrap: true },
          { type: "text", text: `到期時間：${dueLabel}`, size: "sm", color: "#666666" },
          { type: "text", text: `已逾期 ${daysOverdue} 天，請盡快處理。`, size: "sm", color: "#EF4444", wrap: true },
        ],
      },
      footer: actionFooter(task.id, task.confirmation_required),
    },
  };
}

function buildDueSoonFlex(task: TaskWithWorkflow & { due_date: string }, label: string) {
  const wfName = workflowName(task);
  const dueLabel = new Date(task.due_date).toLocaleString("zh-TW", { timeZone: "Asia/Taipei", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" });
  return {
    type: "flex",
    altText: `📢 ${label}：任務「${task.title}」`,
    contents: {
      type: "bubble",
      size: "kilo",
      header: {
        type: "box", layout: "vertical", backgroundColor: "#F59E0B", paddingAll: "14px",
        contents: [
          ...(wfName ? [{ type: "text", text: wfName, size: "xs", color: "#FFF8E1" }] : []),
          { type: "text", text: `📢 ${label}`, weight: "bold", color: "#FFFFFF", size: "md" },
        ],
      },
      body: {
        type: "box", layout: "vertical", spacing: "sm", paddingAll: "14px",
        contents: [
          { type: "text", text: task.title, weight: "bold", size: "md", wrap: true },
          { type: "text", text: `到期時間：${dueLabel}`, size: "sm", color: "#666666" },
        ],
      },
      footer: actionFooter(task.id, task.confirmation_required),
    },
  };
}

serve(async (req: Request) => {
  if (req.method === "OPTIONS") {
    return new Response("ok", { headers: corsHeaders });
  }

  try {
    const supabaseUrl = Deno.env.get("SUPABASE_URL")!;
    const serviceKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
    const lineToken = Deno.env.get("LINE_CHANNEL_ACCESS_TOKEN") || "";
    const sb = createClient(supabaseUrl, serviceKey);

    // Accept optional mode: "all" (default), "reminders", "overdue"
    let mode = "all";
    try {
      const body = await req.json();
      if (body?.mode) mode = body.mode;
    } catch { /* empty body is fine */ }

    const now = new Date().toISOString();
    const today = todayTaipei();
    let reminderCount = 0;
    let overdueCount = 0;
    let skippedCount = 0;

    // ── 1. Process reminder_at notifications (runs on 15-min cron) ──
    if (mode === "all" || mode === "reminders") {
      const { data: reminderTasks } = await sb.from("tasks")
        .select("id, title, due_date, assigned_to, reminder_at, confirmation_required, workflow_instance:workflow_instances(name)")
        .lte("reminder_at", now)
        .eq("reminder_sent", false)
        .not("status", "in", '("completed","cancelled")')
        .limit(50);

      if (reminderTasks && reminderTasks.length > 0) {
        for (const task of reminderTasks) {
          if (!task.assigned_to || !lineToken) {
            // No assignee or no LINE token — mark sent to avoid retrying forever
            await sb.from("tasks").update({ reminder_sent: true }).eq("id", task.id);
            continue;
          }

          const lineId = await resolveLineId(sb, task.assigned_to);
          if (!lineId) {
            skippedCount++;
            console.warn(`Reminder skip: task ${task.id} (${task.title}) — no LINE ID for user ${task.assigned_to}`);
            continue; // Don't mark as sent — retry when user links their LINE
          }

          const sent = await pushLine(lineId, [buildReminderFlex(task)], lineToken);
          if (sent) {
            await sb.from("tasks").update({ reminder_sent: true }).eq("id", task.id);
            await sb.from("task_comments").insert({
              task_id: task.id,
              content: `⏰ 提醒通知已發送 (${new Date().toLocaleString("zh-TW", { timeZone: "Asia/Taipei" })})`,
              source: "system",
            });
            reminderCount++;
          } else {
            console.error(`Reminder failed: task ${task.id} — LINE push failed, will retry`);
          }
        }
      }
    }

    // ── 2. Process overdue notifications (runs weekdays 8am via dedicated cron) ──
    if (mode === "all" || mode === "overdue") {
      const { data: overdueTasks } = await sb.from("tasks")
        .select("id, title, due_date, assigned_to, metadata, confirmation_required, workflow_instance:workflow_instances(name)")
        .lt("due_date", now)
        .not("status", "in", '("completed","cancelled")')
        .limit(50);

      if (overdueTasks && overdueTasks.length > 0) {
        for (const task of overdueTasks) {
          const meta = (task.metadata || {}) as Record<string, unknown>;

          // Skip if already notified today
          if (meta.last_overdue_date === today) continue;

          if (!task.assigned_to || !lineToken) continue;

          const lineId = await resolveLineId(sb, task.assigned_to);
          if (!lineId) {
            skippedCount++;
            console.warn(`Overdue skip: task ${task.id} (${task.title}) — no LINE ID for user ${task.assigned_to}`);
            continue; // Don't mark — no silent failure
          }

          // Calculate days overdue
          const dueDate = new Date(task.due_date);
          const nowDate = new Date();
          const daysOverdue = Math.floor((nowDate.getTime() - dueDate.getTime()) / (1000 * 60 * 60 * 24));

          const sent = await pushLine(lineId, [buildOverdueFlex(task, daysOverdue)], lineToken);
          if (sent) {
            // Mark today's date — allows re-notification tomorrow
            await sb.from("tasks").update({
              metadata: { ...meta, last_overdue_date: today },
            }).eq("id", task.id);

            await sb.from("task_comments").insert({
              task_id: task.id,
              content: `🔴 逾期通知已發送 — 已逾期 ${daysOverdue} 天 (${new Date().toLocaleString("zh-TW", { timeZone: "Asia/Taipei" })})`,
              source: "system",
            });
            overdueCount++;
          } else {
            console.error(`Overdue failed: task ${task.id} — LINE push failed, will retry`);
          }
        }
      }
    }

    // ── 3. Process due-soon notifications (day before + morning of due date) ──
    let dueSoonCount = 0;
    if (mode === "all" || mode === "due_soon") {
      // Get tasks due today or tomorrow (in Taipei timezone)
      const taipeiNow = new Date(new Date().toLocaleString("en-US", { timeZone: "Asia/Taipei" }));
      const todayStr = todayTaipei();
      const tomorrowDate = new Date(taipeiNow);
      tomorrowDate.setDate(tomorrowDate.getDate() + 1);
      const tomorrowStr = `${tomorrowDate.getFullYear()}-${String(tomorrowDate.getMonth() + 1).padStart(2, '0')}-${String(tomorrowDate.getDate()).padStart(2, '0')}`;

      const { data: dueSoonTasks } = await sb.from("tasks")
        .select("id, title, due_date, assigned_to, metadata, confirmation_required, workflow_instance:workflow_instances(name)")
        .gte("due_date", todayStr)
        .lt("due_date", `${tomorrowStr}T23:59:59`)
        .not("status", "in", '("completed","cancelled")')
        .limit(50);

      if (dueSoonTasks && dueSoonTasks.length > 0) {
        for (const task of dueSoonTasks) {
          if (!task.assigned_to || !lineToken || !task.due_date) continue;

          const meta = (task.metadata || {}) as Record<string, unknown>;
          const dueDateStr = task.due_date.slice(0, 10);
          const isDueToday = dueDateStr === todayStr;
          const isDueTomorrow = dueDateStr === tomorrowStr;

          // Determine which notification to send
          let label = "";
          let metaKey = "";
          if (isDueTomorrow && meta.due_soon_tomorrow !== todayStr) {
            label = "明日到期提醒";
            metaKey = "due_soon_tomorrow";
          } else if (isDueToday && meta.due_soon_today !== todayStr) {
            label = "今日到期提醒";
            metaKey = "due_soon_today";
          } else {
            continue; // Already notified
          }

          const lineId = await resolveLineId(sb, task.assigned_to);
          if (!lineId) {
            skippedCount++;
            continue;
          }

          const sent = await pushLine(lineId, [buildDueSoonFlex(task as any, label)], lineToken);
          if (sent) {
            await sb.from("tasks").update({
              metadata: { ...meta, [metaKey]: todayStr },
            }).eq("id", task.id);
            await sb.from("task_comments").insert({
              task_id: task.id,
              content: `📢 ${label}已發送 (${new Date().toLocaleString("zh-TW", { timeZone: "Asia/Taipei" })})`,
              source: "system",
            });
            dueSoonCount++;
          }
        }
      }
    }

    return new Response(JSON.stringify({
      ok: true,
      mode,
      reminders_sent: reminderCount,
      overdue_sent: overdueCount,
      due_soon_sent: dueSoonCount,
      skipped_no_line_id: skippedCount,
    }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (err) {
    console.error("task-reminder error:", err);
    return new Response(JSON.stringify({ error: String(err) }), {
      status: 500,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
});
